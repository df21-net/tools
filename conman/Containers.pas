...
begin
 ...
 begin
  ...
  Fi.offs:=F.Fpos;
  if (CompareText(n,'Resource')<>0) or
     (CompareText(e,'RMAP')<>0) then
...
Try
 ...
 begin
  ...
  begin
   ...
   repeat
    psl:=StrScan(psl,'\');
    if psl=nil then AddDir(ps)
    else begin AddDir(Copy(ps,1,psl-ps)); inc(psl); end;
  until psl=nil;
...
begin
 Result:=TGOB2Creator.Create(Name);
end;
